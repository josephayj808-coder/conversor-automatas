...
class Estado:
    ...
    garantizando que se modele correctamente el No-Determinismo (múltiples caminos por símbolo).
    """
...
class AFN:
    ...
    def clausura_positiva(self, gen_id):
        # OPERACIÓN: Clausura Positiva (R+).
        # Matemáticamente equivale a R.R*, exigiendo al menos una iteración obligatoria.
        # Estructuralmente, es idéntica a la estrella de Kleene pero omitiendo la transición λ 
        # directa desde el nuevo_inicial hasta el nuevo_final, impidiendo aceptar la cadena vacía
        # a menos que el bloque interno R ya la contenga.
        nuevo_inicial = Estado(gen_id())
        nuevo_final = Estado(gen_id())
...
# Se ha agregado el operador '+' al conjunto de símbolos soportados.
...
def formatear_regex(regex):
    ...
    for i in range(len(regex)):
        ...
        if i + 1 < len(regex):
            c2 = regex[i+1]
            # '+' también detona una concatenación implícita hacia el siguiente símbolo
...
def infija_a_postfija(regex):
    ...
    Ambos operadores unarios (* y +) comparten el máximo nivel de precedencia.
    """
    precedencia = {'*': 3, '+': 3, '.': 2, '|': 1}
    salida = []
    pila = []
...
def construir_afn(postfija):
    """
    Máquina de pila que evalúa la cadena postfija y ensambla el autómata.
    """
    pila = []
...
regex_input = st.text_input("Ingresa la Expresión Regular (usa '|' para unión, '*' para Kleene, '+' para C. Positiva):")
